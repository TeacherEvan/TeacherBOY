...
def mock_brave_service():
    ...
    service.search = AsyncMock(
        return_value=[{"title": "Python Tutorial", "url": "https://python.org", "description": "Learn Python"}]
    )
    return service
...
@pytest.mark.asyncio
async def test_should_not_handle_search_in_group_for_non_admin(search_agent, group_message_event):
    assert await search_agent.should_handle(group_message_event, "Ms. Green search python") is True
...
@pytest.mark.asyncio
async def test_should_handle_search_in_group_even_when_legacy_allowlist_denies(search_agent, group_message_event, monkeypatch):
    ...
    monkeypatch.setattr(search_agent_module.settings, "zeus_group_access_mode", "allowlist", raising=False)
    monkeypatch.setattr(
