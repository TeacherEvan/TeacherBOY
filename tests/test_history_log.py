...
class TestZeusErrorMessages:
    ...
    def test_api_failure_message(self):
        ...
        msg = ZeusErrorMessages.get_error_message("api_failure", error="Connection timeout", service="OpenRouter")

        assert (
            "OLYMPUS" in msg.upper()
            or "ZEUS" in msg.upper()
            or "THUNDERBOLT" in msg.upper()
            or "BETRAYED" in msg.upper()
            or "COLLAPSED" in msg.upper()
        )
        assert "Connection timeout" in msg or "OpenRouter" in msg

    def test_network_error_message(self):
        """Test network error message generation."""
        msg = ZeusErrorMessages.get_error_message("network_error", error="DNS resolution failed")
    ...
    def test_auth_failure_message(self):
        ...
        msg = ZeusErrorMessages.get_error_message("auth_failure", error="Invalid API key")
    ...
    def test_rate_limit_message(self):
        ...
        msg = ZeusErrorMessages.get_error_message("rate_limit", error="429 Too Many Requests", wait="60 seconds")
...
class TestHistoryLogService:
    ...
    async def test_error_logging_with_zeus_style(self, log_service):
        ...
        assert any(
            word in entry.message.upper()
            for word in ["OLYMPUS", "THUNDERBOLT", "MORTAL", "ZEUS", "FAILED", "COLLAPSED", "BETRAYED", "DIVINE"]
        )
    ...
    async def test_query_logs(self, log_service):
        ...
        # Query by event type
        user_messages = await log_service.query_logs(event_types=[EventType.USER_MESSAGE])
        assert len(user_messages) == 2

        # Query by level
        errors = await log_service.query_logs(levels=[LogLevel.ERROR])
        assert len(errors) == 1

        # Query by agent
        agent1_logs = await log_service.query_logs(agent_name="Agent1")
