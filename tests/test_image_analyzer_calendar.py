...
class TestImageAnalyzerCalendarFriendCheck:
    ...
    async def test_non_friend_gets_quirky_rejection(
        ...
        mock_line_bot_api.get_profile = MagicMock(
        ...
        image_analyzer_session_manager.store_detected_dates(
            chat_id,
            [{"date": "2026-01-15", "title": "Test Event", "description": "Test"}]
        )
        ...
        assert any(keyword in message_text.lower() for keyword in [
            "friend", "friends", "เพื่อน"
        ])

    @pytest.mark.asyncio
    async def test_friend_can_add_to_calendar(
        self, image_analyzer_agent, mock_event, mock_line_bot_api
    ):
    ...
    @pytest.mark.asyncio
    async def test_admin_bypasses_friend_check(
        self, image_analyzer_agent, mock_event, mock_line_bot_api
    ):
    ...
    @pytest.mark.asyncio
    async def test_skip_calendar_works_for_non_friends(
        self, image_analyzer_agent, mock_event, mock_line_bot_api
    ):
