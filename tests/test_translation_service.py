...
class TestTranslationService:
    ...
    async def test_detect_language_failure(self, service):
        ...
        with patch('src.services.translation_service.detect', side_effect=Exception("Error")):
            lang = await service.detect_language(text)
            assert lang is None
    ...
    async def test_auto_translate_thai_to_english(self, service):
        ...
        with patch.object(service, 'detect_language', return_value='th'), \
             patch.object(service, 'translate', return_value=expected_translation):
