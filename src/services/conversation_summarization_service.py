...
Keep the summary concise but complete. Focus on information that would be valuable for continuing the conversation."""
    ...
    async def should_summarize(
        ...
        if len(messages) >= message_threshold:
            logger.info(f"📝 Conversation exceeds message threshold: {len(messages)} >= {message_threshold}")
            return True
        ...
        if estimated_tokens >= token_threshold:
            logger.info(f"📝 Conversation exceeds token threshold: ~{estimated_tokens} >= {token_threshold}")
            return True
    ...
    async def summarize_conversation(
        ...
        try:
            ...
            logger.info(f"📝 Summarizing {len(messages_to_summarize)} messages (preserving {preserve_recent} recent)")
    ...
    def create_compressed_history(
        ...
        logger.info(
            f"📝 Created compressed history: {len(compressed_history)} messages (summary + {len(recent_messages)} recent)"
        )
