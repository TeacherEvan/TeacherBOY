...
class PromptBuilderService:
    ...
    def build_prompt(
        ...
        if core_tokens > available_tokens:
            logger.warning(
                f"⚠️ Core prompt ({core_tokens} tokens) exceeds budget ({available_tokens} tokens). Truncating user message."
            )
    ...
    def create_conversation_context(
        ...
        return ContextBlock(
            content=context, priority=priority, label="conversation_history", metadata={"message_count": len(recent_messages)}
        )
    ...
    def create_user_profile_context(
        ...
        return ContextBlock(content=context, priority=priority, label="user_profile", metadata={"user_id": user_id})
    ...
    def create_system_state_context(
        ...
        return ContextBlock(
            content=context, priority=priority, label="system_state", metadata={"timestamp": current_time.isoformat()}
        )
