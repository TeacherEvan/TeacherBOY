...
class CalendarAccessControl:
    ...
    async def get_user_role(
        self, user_id: str, chat_id: str, event_owner_id: str | None = None, line_bot_api: MessagingApi | None = None
    ) -> CalendarRole:
        """
        ...
        if chat_id.startswith("group_") or chat_id.startswith("room_"):
            is_member = await self._verify_chat_membership(user_id, chat_id, line_bot_api)
    ...
    async def can_view_events(self, user_id: str, chat_id: str, line_bot_api: MessagingApi | None = None) -> bool:
        """
    ...
    async def can_create_event(self, user_id: str, chat_id: str, line_bot_api: MessagingApi | None = None) -> bool:
        """
    ...
    async def can_modify_event(
        self, user_id: str, chat_id: str, event_owner_id: str, line_bot_api: MessagingApi | None = None
    ) -> bool:
        """
        ...
        """
        role = await self.get_user_role(user_id, chat_id, event_owner_id=event_owner_id, line_bot_api=line_bot_api)
    ...
    async def _verify_chat_membership(self, user_id: str, chat_id: str, line_bot_api: MessagingApi | None) -> bool:
        """
        ...
        try:
            ...
            if chat_id.startswith("group_"):
                ...
                profile = await asyncio.to_thread(line_bot_api.get_group_member_profile, group_id, user_id)
                return profile is not None

            elif chat_id.startswith("room_"):
                room_id = chat_id.replace("room_", "")
                # Attempt to get room member profile
                profile = await asyncio.to_thread(line_bot_api.get_room_member_profile, room_id, user_id)
                return profile is not None
