...
class ChatBuffer:
    ...
    messages: deque[BufferedMessage] = field(default_factory=lambda: deque(maxlen=DEFAULT_MAX_MESSAGES))
    ...
    def get_messages(self, limit: int = 10, ttl_seconds: int = DEFAULT_TTL_SECONDS) -> list[BufferedMessage]:
        """
        ...
        valid_messages = [msg for msg in self.messages if not msg.is_expired(ttl_seconds)]
...
class MessageBufferService:
    ...
    def __init__(
        ...
        logger.info(f"📝 MessageBufferService initialized (max={max_messages_per_chat}, ttl={message_ttl_seconds}s)")
    ...
    def store_message(
        ...
        with self._lock:
            if chat_id not in self._buffers:
                self._buffers[chat_id] = ChatBuffer(messages=deque(maxlen=self._max_messages))
        ...
        logger.debug(f"📝 Buffered message in {chat_id}: '{text[:30]}...' from {user_id}")
    ...
    def get_recent_messages(
        ...
        with self._lock:
            ...
            if exclude_user_id:
                messages = [msg for msg in messages if msg.user_id != exclude_user_id][:limit]
    ...
    def get_message_texts(
        ...
        """
        messages = self.get_recent_messages(chat_id, limit, exclude_user_id)
    ...
    def get_buffer_stats(self, chat_id: str) -> dict[str, int]:
        ...
        with self._lock:
            ...
            valid = len(buffer.get_messages(limit=total, ttl_seconds=self._ttl_seconds))

            return {"total_messages": total, "valid_messages": valid}
