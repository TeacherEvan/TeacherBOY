...
class ImageConsentService:
    ...
    def __init__(self, owner_user_ids: Optional[Iterable[str]] = None) -> None:
        self._owner_user_ids: Set[str] = {
            user_id for user_id in (owner_user_ids or []) if user_id
        }

    def is_owner_scoped_user(self, user_id: Optional[str]) -> bool:
        return bool(user_id and user_id in self._owner_user_ids)

    def is_consented_owner(self, user_id: Optional[str]) -> bool:
        if not user_id:
            return False
