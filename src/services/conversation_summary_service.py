...
class ConversationSummarizer:
    ...
    def __init__(
        ...
        logger.info(
            f"📝 Summarizer initialized: threshold={max_tokens_before_summary}, "
            f"keep_recent={messages_to_keep_full}"
        )
    ...
    async def maybe_summarize(
        ...
        logger.debug(
            f"📝 Conversation size: {len(messages)} messages, "
...
Provide a concise summary (max 150 words):"""
    ...
    def estimate_savings(
        ...
        recent_tokens = sum(
            len(m.get("content", "")) // 4 
            for m in messages[-self.keep_recent:]
        )
