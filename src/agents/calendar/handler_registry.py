...
class HandlerRegistry:
    ...
    def get_handler(self, handler_key: str) -> Optional[CalendarHandler]:
        """
    ...
    def _load_handler(self, handler_key: str) -> Optional[CalendarHandler]:
        """
    ...
    def _mark_failed(self, handler_key: str) -> None:
        ...
        logger.warning(
            f"Handler {handler_key} marked as failed. "
            f"Will retry after {self._failure_threshold.total_seconds()}s"
        )
...
_registry_instance: Optional[HandlerRegistry] = None
