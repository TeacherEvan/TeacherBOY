...
try:
    ...
    logger.warning(
        "⚠️  Google Calendar API libraries not installed. Run: pip install google-api-python-client google-auth-oauthlib"
    )
    ...
    def _create_daily_reminder_flex(self, events: List[Dict[str, Any]]) -> FlexMessage:
        ...
        if not events:
            ...
            for i, event in enumerate(events):
                ...
                if i < len(events) - 1:
                    event_contents.append({"type": "separator", "margin": "md"})
        ...
        if not events:
            ...
            for i, date_str in enumerate(sorted_dates):
                ...
                if i < len(sorted_dates) - 1:
                    day_contents.append({"type": "separator", "margin": "lg"})
    ...
    async def send_daily_reminder(self, line_bot_api: MessagingApi):
        ...
        try:
            ...
            line_bot_api.push_message(
                PushMessageRequest(to=self.group_chat_id, messages=[flex_message])
            )
